...
import { Text, Stack, Dropdown, mergeStyleSets, IStackStyles, IDropdownOption, PivotItem } from "@fluentui/react";
...
const Home = memo(function Home() {
  ...
  const onSeqChange = useCallback(
    (_ev: any, option: IDropdownOption) => {
      const url: UrlObject = {
        pathname: "/",
        query: { tab, sequence: option.text }
      };
      router.push(url, url, { shallow: true });
    },
    [tab]
  );
  const options = useMemo<IDropdownOption[]>(() => seqs.map((seq) => ({ key: seq, text: seq })), [seqs]);
  ...
  return (
    <Stack styles={wrapperStyles}>
      <Stack styles={headerStyles} horizontal>
        ...
        <Dropdown options={options} selectedKey={seqName} dropdownWidth="auto" onChange={onSeqChange} />
      </Stack>
      <Stack horizontal grow={1} styles={mainWrapperStyles}>
        <Stack disableShrink styles={sidebarStyles}>
          ...
          {tab == "2d" && <LevaPanel store={store2} fill flat hideCopyButton titleBar={false} theme={levaTheme} />}
        </Stack>
      ...
      {tab === "3d" && (
        <>
          <div className={classes.controlsWrapper} ref={levaPanelRef} onMouseDown={clickLevaPanel} onMouseUp={leaveLevaPanel}>
